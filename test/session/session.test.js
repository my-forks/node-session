		...
		.addBatch(
				{
					"get() " : {
						...
						'should return undefined if key does not exists' : function(
								topic) {
							assert.equal(topic.get('fldjslfkjdlks'), undefined);
						},
						'should return defaultValue if key does not exists' : function(
								topic) {
							assert.equal(topic.get('fldjslfkjdlks',
					...
					"set() " : {
						...
						'should return this' : function(topic) {
							assert.equal(topic.set('fldjslfkjdlks', 'jkljlm'),
									topic);
						},
		...
		.addBatch(
				{
					"generateId()" : {
						...
						'should generate SID with length 32bytes(chars).' : function(
								topic) {
					...
					"create()" : {
						...
						'should set appropriate createdAt to current time.' : function(
								topic) {
							var sessionNew = topic.create();
							assert.equal(sessionNew.createdAt, getTime());
							assert.equal(sessionNew.accessedAt, getTime());
							assert.equal(sessionNew.expiredAt, getTime()
									+ TIME_EXPIRE);
						}
					},
					"exist()" : {
						...
						'should return false if SID does not exists' : function(
								topic) {
						...
						'should return true if Session has expired' : function(
								topic) {// TODO return true OR false and auto
					...
					"open()" : {
						...
						'should set new SID if SID does not exists' : function(
								topic) {
						...
						'should set new SID if Session has expired' : function(
								topic) {
							var sessionNew = topic.create();
						...
						'should return Session object if SID exists' : function(
								topic) {
							var sessionNew = topic.create();

							var sessionOpened = topic.open(sessionNew.getId());
							assert.notEqual(sessionOpened, undefined);
							assert.equal(sessionOpened.getId(), sessionNew
									.getId());
							assert.deepEqual(sessionOpened._getData(),
									sessionNew._getData());

							assert.equal(sessionOpened.createdAt,
									sessionNew.createdAt);
							assert.equal(sessionOpened.accessedAt,
									sessionNew.accessedAt);
							assert.equal(sessionOpened.expiredAt,
									sessionNew.expiredAt);
						},

						'should save new data after Session.save()' : function(
								topic) {
							var sessionNew = topic.create();
							var sid = sessionNew.getId();
						...
						'should set access and expiration time after Session.save()' : function(
								topic) {
							var sessionNew = topic.create();
							var sid = sessionNew.getId();
							...
							assert.equal(sessionOpened.expiredAt, getTime()
									+ TIME_EXPIRE);
						}
					},
					"destroy()" : {
						...
						'should not send an error if SID does not exists' : function(
								topic) {
							assert.doesNotThrow(function() {
								var session = topic.create();
						...
						'should return destroy session if exists' : function(
								topic) {
							var session = topic.create();
							assert.equal(topic.exist(session.getId()), true);
							topic.destroy(session);
					...
					"flush()" : {
						...
						'should not throw any errors if no session' : function(
								topic) {
							assert.doesNotThrow(function() {
								topic.flush();
							}, topic);
						...
						'should destroy all sessions' : function(topic) {
							...
							topic.flush();
							sessions
									.forEach(function(sessionCreated) {
										assert.ok(!topic.exist(sessionCreated
												.getId()));
									});
						}
					},
